...
1. 启动后进入**模板对话**，逐个回答故事构想（类型、主角、世界观、基调等）
2. 模板完成后输入「开始」生成完整开局设计
3. `/accept` 确认开局，或输入修改意见让导播调整
4. 每个场景流式播出后给出走向候选，选择或直接描述
5. `/mode auto` 切换挂机自动模式
...
- **LLM 服务商**：商汤 SenseNova / OpenAI 兼容（DeepSeek、通义、Kimi…）/ 本地 llama.cpp，按世界（工作区）持久化。
- **按角色覆盖模型 / 温度**（高级，默认收起）：导播 / 执笔 / 校对可分别指定模型与采样温度；空白即回退全局配置。
- **测试连接**：保存前做一次最小连通性验证。
- **导出**：`导出 TXT` / `导出 MD` / `导出 HTML` —— 整本已写场景，只读不落盘（HTML 为单文件离线阅读页）。
- 生成过程中顶栏出现 **■ 停止**，可随时中断；进度停留在上一个稳定点，重启后自动续写。
