...
src/
  ...
  engine/    引擎主循环（engine.ts）、上下文组装（context.ts）、结算（settle.ts）
...
3. **提交门槛**：`npm run typecheck` 通过 + `npm run e2e` 通过。纯前端改动至少在 mock 模式（`npm run web:mock`）过一遍核心流程。
4. **运行时数据不入库**：工作区目录、日志、.env 永远 gitignore；`.env.example` 是唯一配置模板。
5. **服务端零依赖**：Web 服务只用 node:http（见 server.ts 头注释），新增后端能力优先保持零依赖。
